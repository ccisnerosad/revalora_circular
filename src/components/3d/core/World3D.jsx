...
const World3D = memo(function World3D({ selectedZone, onSelect, onHover, showFlows, viewMode, targetPosition }) {
  ...
  return (
    <>
      ...
      {/* Zonas Arquitectónicas Planta Baja */}
      <group>
      ...
      <group>
        {ZONES_DATA_PA.map((zone) => (
          <Zone key={zone.id} data={zone} isSelected={selectedZone === zone.id} isAnySelected={selectedZone !== null} onSelect={() => onSelect(zone.id)} onHover={onHover} viewMode={viewMode} />
        ))}
      </group>
