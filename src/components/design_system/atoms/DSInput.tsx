...
export const DSInput = (props: UniversalFieldProps) => {
  const { inputProps, status, error, isLive } = useFieldMode(props);

  // Iconos de estado para modo Live
  const StatusIcon = () => {
    ...
    if (status === 'error') {
      return (
        <div className="absolute right-3 top-1/2 -translate-y-1/2 text-red-500" title="Error al guardar">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5">
      ...
      <div className="relative">
        <input
          ...
          className={twMerge(
            ...
            props.mode === 'preview' && "bg-transparent border-transparent shadow-none px-0"
          )}
        />
        <StatusIcon />
      </div>
